...
CWmin = 0: 1: 7; %contention window parameter- 2^CWminN
CWmaxN = 7; 
...
packet_loss = zeros(3, length(CWmin));
throughput = zeros(3, length(CWmin));
buffutil = zeros(3, length(CWmin));

for lm = 1:length(CWmin)
    CWminN = CWmin(lm);
...
figure(2);
plot(CWmin, packet_loss(1, :), '--*r');
hold on;
plot(CWmin, packet_loss(2, :), '--*g');
hold on;
title('packet loss rate to CWmin');
xlabel('CWmin');
ylabel('packet loss rate (%)');
legend('downlink DCF', 'uplink DCF');
hold off;
figure(3);
plot(CWmin, throughput(1, :), '--*r');
hold on;
plot(CWmin, throughput(2, :), '--*g');
hold on;
title('throughput to CWmin');
xlabel('CWmin');
ylabel('throughput (bit/s)');
legend('downlink DCF', 'uplink DCF');
hold off;
figure(4);
plot(CWmin, buffutil(1, :), '--*r');
hold on;
plot(CWmin, buffutil(2, :), '--*g');
hold on;
title('buffer utilization rate to CWmin');
xlabel('CWmin');
