...
xlabel('lambda (bit/s)');
ylabel('packet loss probability');
poll_UL_packet_loss = poll_UL_packet_loss ./ poll_total_UL_packet;
figure(4);
bar(uplink_lambda, poll_UL_packet_loss);
title('poll UL packet loss to lambda');
xlabel('lambda (bit/s)');
ylabel('packet loss probability');
poll_DL_packet_loss = poll_DL_packet_loss ./ poll_total_DL_packet;
figure(5);
bar(uplink_lambda, poll_DL_packet_loss);
title('poll DL packet loss to lambda');
xlabel('lambda (bit/s)');
ylabel('packet loss probability');