function [capacity, SINR] = calculate_LTE_DL_channel_state(MS, AP, APofMS, shift, side, MS_frequency, AP_frequency, bandwidth, frequency_reuse_factor, NumFDMA, downlink_buffer)
    ...
    for i = 1: length(AP)
        ...
        [capacity(now), SINR(now)] = calculate_LTE_DL_SINR(MS(now), AP(i), AP(APofMS(now)), shift, side, bandwidth, frequency_reuse_factor, NumFDMA);
