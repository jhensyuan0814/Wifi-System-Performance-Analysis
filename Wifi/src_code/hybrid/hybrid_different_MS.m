clear all;
clear plt;
MSperCell_collection = 4: 2: 16;
packet_loss = zeros(3, length(MSperCell_collection));
throughput = zeros(3, length(MSperCell_collection));
buffutil = zeros(3, length(MSperCell_collection));
for ms = 1:length(MSperCell_collection)
    %set some parameters
    bandwidth = 10 ^ 7;
    side = 1000; %length of square boundary 
    APNum = 16; %total number of APs
    MSperCell = MSperCell_collection(ms);  %average number of MSs per AP
    ...
    MSlimit = MSperCell; %the maximum number of MSs under one AP
    ...
    packet_loss(1, ms) = downlink_packet_loss / (downlink_total_packet - sum(downlink_buffer));
    packet_loss(2, ms) = uplink_packet_loss / (uplink_total_packet - sum(uplink_buffer));
    packet_loss(3, ms) = poll_packet_loss / poll_total_packet;
    throughput(1, ms) = (downlink_total_packet-sum( downlink_buffer))* payload_size / iteration_time / simulation_time;
    throughput(2, ms) = (uplink_total_packet-sum( uplink_buffer))* payload_size / iteration_time / simulation_time;
    throughput(3, ms) = poll_total_packet* payload_size / iteration_time / simulation_time;
    buffutil(1, ms) = sum(downlink_buffer) * payload_size/ downlink_buffer_size*payload_size / length(downlink_buffer) * 100;
    buffutil(2, ms) = sum(uplink_buffer) * payload_size/ uplink_buffer_size / length(uplink_buffer) * 100;
end
figure(2);
packet_loss = packet_loss * 100;
plot(MSperCell_collection, packet_loss(1, :), '--*r');
hold on;
plot(MSperCell_collection, packet_loss(2, :), '--*g');
hold on;
plot(MSperCell_collection, packet_loss(3, :), '--*b');
hold on;
title('packet loss to number of MS per Cell');
legend('downlink', 'uplink DCF', 'uplink PCF');
xlabel('number of MS per Cell');
ylabel('packet loss rate (%)');
hold off;
figure(3);
plot(MSperCell_collection, throughput(1, :), '--*r');
hold on;
plot(MSperCell_collection, throughput(2, :), '--*g');
hold on;
plot(MSperCell_collection, throughput(3, :), '--*b');
hold on;
...
hold off;
figure(4);
plot(MSperCell_collection, buffutil(1, :), '--*r');
hold on;
plot(MSperCell_collection, buffutil(2, :), '--*g');
hold on;
title('buffer utilization to number of MS per Cell');
