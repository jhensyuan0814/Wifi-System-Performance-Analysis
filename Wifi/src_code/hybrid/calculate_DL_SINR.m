function [capacity,SINR] = calculate_DL_SINR(MS, AP, APofMS, shift, side, bandwidth, frequency_reuse_factor, NumFDMA)
    ...
    for i = 1: numberMS
        ...
        tot_downlink(i) = sum(t_sinr);
    end
    for k = 1: numberMS
        %wrap-around style to calculate SINR
        a = abs(real(AP_collection - MS(k))) < side / 2;
        b = abs(imag(AP_collection - MS(k))) < side / 2;
        t_AP = AP_collection(a & b);
