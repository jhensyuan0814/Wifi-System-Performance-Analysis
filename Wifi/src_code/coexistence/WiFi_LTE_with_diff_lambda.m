...
LTE_pkt_loss = [];
use_fac_record = [];
lambda_i = [0.001:0.001:0.01];
packet_loss = zeros(4, length(lambda_i));
throughput = zeros(4, length(lambda_i));
buffutil = zeros(4, length(lambda_i));
for lm = 1: length(lambda_i)
    %set some parameters
    bandwidth = 10 ^ 7;
    ...
    downlink_lambda = lambda_i(lm);
    uplink_lambda = lambda_i(lm);%1000;%1000
    ...
    %clearvars temp1 temp2 temp3 1: wifi 2: LTE
    packet_loss(1, lm) = sum(downlink_lostMS(1,:))/(downlink_pkt_count(1)) * 100;
    packet_loss(2, lm) = sum(downlink_lostMS(2,:)/(downlink_pkt_count(2))) * 100;
    throughput(1, lm) = downlink_pkt_count(1) * payload_size / iteration_time / simulation_time;
    throughput(2, lm) = downlink_pkt_count(2)* payload_size / iteration_time / simulation_time;
    buffutil(1, lm) = sum(downlink_buffer(1,:)) * payload_size/ downlink_buffer_size / length(downlink_buffer) * 100;
    buffutil(2, lm) = sum(uplink_buffer(1,:)) * payload_size/ uplink_buffer_size / length(uplink_buffer) * 100;
    disp("downlink wifi")
...
%plot histogram
lambda_i = lambda_i * payload_size / iteration_time;
figure(2);
plot(lambda_i, packet_loss(1, :), '--*r');
hold on;
plot(lambda_i, packet_loss(2, :), '--*g');
hold on;
title('packet loss rate to lambda');
legend('downlink DCF', 'downlink LTE');
xlabel('lambda (bit/s)');
ylabel('packet loss rate (%)');
hold off;
figure(3);
plot(lambda_i, throughput(1, :), '--*r');
hold on;
plot(lambda_i, throughput(2, :), '--*g');
hold on;
...
hold off;
figure(4);
plot(lambda_i, buffutil(1, :), '--*r');
hold on;
plot(lambda_i, buffutil(2, :), '--*g');
hold on;
title('buffer utilization rate to lambda');
legend('downlink DCF', 'downlink LTE');
xlabel('lambda (bit/s)');
